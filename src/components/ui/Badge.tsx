...
const statusConfig: Record<BadgeStatus, { label: { es: string; en: string }; className: string; Icon: typeof CheckCircle }> = {
  ...
  error: {
    label: { es: 'Documento inválido', en: 'Invalid document' },
  ...
  warning: {
    label: { es: 'Advertencia', en: 'Warning' },
    className: 'bg-amber-50 text-amber-700 border border-amber-200',
    Icon: AlertTriangle
  },
  review: {
    label: { es: 'Revisión requerida', en: 'Review required' },
    className: 'bg-amber-50 text-amber-700 border border-amber-200',
    Icon: Clock
  },
  na: {
    label: { es: 'No aplica', en: 'Not applicable' },
    className: 'bg-slate-100 text-slate-600 border border-slate-200',
    Icon: Clock
  }
};

export function StatusBadge({ status, language = 'es' }: { status: ValidationStatus | BadgeStatus; language?: 'es' | 'en' }) {
